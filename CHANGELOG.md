...
### Added
- **OpenRouter hub:** when `OPENROUTER_API_KEY` is set, the gateway advertises `openrouter/<vendor>/<model>`, quotes cost-plus (upstream per-token price + 1% + the $0.002 receipt, never below upstream, `max_tokens` capped), and records the requested model plus the served route. Every OpenRouter request sends `HTTP-Referer: https://chit402.com` and `X-Title: Chit402` (`OPENROUTER_REFERER` / `OPENROUTER_TITLE`). The hub stays off when the key is unset.
