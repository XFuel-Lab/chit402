...
### Added
- **OpenRouter hub:** when `OPENROUTER_API_KEY` is set, the gateway advertises `openrouter/<vendor>/<model>`, quotes cost-plus (upstream per-token price + 1% + the $0.002 receipt, never below upstream, `max_tokens` capped), and records the requested model plus the served route. Every OpenRouter request sends `HTTP-Referer: https://chit402.com`, `X-OpenRouter-Title: Chit402` (and `X-Title`), and `X-OpenRouter-Categories: cloud-agent`. `X-OpenRouter-App-Visibility` is not sent. After a completion, `GET /generation?id=` may record `total_cost` and `upstream_inference_cost` on the receipt without blocking the response. The hub stays off when the key is unset.
