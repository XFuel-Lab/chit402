...
thesis. See `packages/agent-skills/_shared/reference/payments-x402.md`.
...
## Settlement model (token-light, on Base)

USDC lands at a single **Base** address (`X402_PAY_TO` / Splits v2; ADR 0001). Fan-out
is off the hot path and governance-adjustable — there is **no** hardcoded 30/30/25/15
split. The payer is **agent-side / pluggable** (no server-custodial keys). Optional
legacy native rails may exist for specific provider flows but are not the default.
...
X402_FALLBACK_TFUEL=false         # keep false for live x402 demos (true = fall back to TFUEL vs 503)

# Facilitator protocol selection
X402_FACILITATOR_PROVIDER=x402    # 'x402' (standard/public; default & live testnet) | 'zan' (bespoke, optional)
...
1. **`/task-request` 402 handshake** behind `X402_ENABLED` — ✅ **live**: returns the
   challenge when `rail=usdc` and no `X-PAYMENT`; verifies+settles on retry; attaches
   `paymentRail`/`paymentRef` to the task + status + `TaskSettled` webhook.
2. **`/task-quote`** endpoint using `priceTaskUSDC` — ✅ **live**.
...
4. **Base mainnet** — ⛔ not yet: needs a mainnet-capable facilitator (e.g. Coinbase
   CDP). **Phase 2 (started):** commit `paymentRef` into SP1 public values so the proof
   attests paid+computed (see below).
...
- **Flag:** `X402_PROOF_BINDING` (default off; **enabled on the live demo box** —
  server-attested `in_proof:false` until the SP1 guest ships the v2 layout). When on, for a USDC-settled task the
...
| 5 | Smoke test | Run `packages/sdk/examples/pay-prove-verify.ts` — expect `payment_binding.in_proof: true` and matching `ai_public_values_abi`. |
