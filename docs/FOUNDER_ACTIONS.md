...
Last updated: 2026-08-15 · Public Base mainnet x402 live · Repo on `main` @ `e9dc2c2`; **the live box is still on `20fa5d6`** and prices on the old rate card · SDK `0.5.2` · SP1 prover running · **Cost-plus + Tier-2 at $0.08 accepted ([ADR 0009](./adr/0009-cost-plus-pricing.md)), shipped default-off** · Next: item 27 (deploy), then outreach (4) — traffic is currently 0 tasks/24h, which is the real constraint
...
| 27 | **Nothing you must do — but know that merging #175 changed no prices.** Cost-plus ships default-off, so the live box still meters the old rate card at `default_bps: 50` until `X402_COST_PLUS=true` and `VI_TIER2_MIN_COGS=2000000` are set **on the box** and it restarts. The two are only safe together; `checkPricingConfig` errors at boot if they are separated | Deploy checklist is the item 3c table below. Worth knowing the traffic context before pricing another round: `/stats` on 2026-08-15 read **6 paid tasks in 7d, $0 USDC fees in 7d, 0 tasks in 24h** — the pricing model currently governs approximately no revenue, so item 4 (outreach) gates everything downstream of it |
