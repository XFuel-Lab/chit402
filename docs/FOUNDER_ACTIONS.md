...
Last updated: 2026-08-15 · Public Base mainnet x402 live · Repo on `main` @ `e9dc2c2`; **the live box is still on `20fa5d6`** and prices on the old rate card · SDK `0.5.2` · SP1 prover running · **Cost-plus + Tier-2 at $0.08 accepted ([ADR 0009](./adr/0009-cost-plus-pricing.md)); the engine is now wired to the money path, still default-off** · Next: item 27 (deploy), then outreach (4) — traffic is currently 0 tasks/24h, which is the real constraint
...
| 21 | **Accept [ADR 0008](./adr/0008-rolling-settlement.md)** — charge the previous call's *measured* cost on the next request instead of quoting output at `max_tokens`. Kills the overcharge (15x on a ceiling-heavy call) with **no** x402 v2 / Permit2 migration, which is what has kept the `upto` fix open for weeks | Built and tested behind `X402_ROLLING_SETTLEMENT`, **default off**. Three calls for you in the ADR: which surface goes first, whether one-call-per-payer bad debt at $1 max is acceptable, and whether a durable ledger lands before this counts as revenue (recommended: yes — pending charges are in memory today, so a restart forgives them) |
...
| 27 | **Deploy before re-enabling cost-plus.** `X402_COST_PLUS=true` was set on the box on 2026-08-15 and had to be reverted within the hour: on the deployed build the flag moved the *advertised* price and not the charged one, publishing $1.54/$4.84 per million while billing $3.00/$9.00. The engine is wired now, but only on `main` — setting the flag again before the box pulls repeats the same 2x overcharge. Order is: pull, restart, confirm `/task-quote` returns `basis: "cost_plus"`, *then* set the flag. It stays safe only alongside `VI_TIER2_MIN_COGS=2000000`; `checkPricingConfig` errors at boot if they are separated | Deploy checklist is the item 3c table below. Worth knowing the traffic context before pricing another round: `/stats` on 2026-08-15 read **6 paid tasks in 7d, $0 USDC fees in 7d, 0 tasks in 24h** — the pricing model currently governs approximately no revenue, so item 4 (outreach) gates everything downstream of it |
