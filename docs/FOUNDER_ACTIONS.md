...
| 21 | **Accept [ADR 0008](./adr/0008-rolling-settlement.md)** — charge the previous call's *measured* cost-plus bill on the next request instead of quoting output at `max_tokens`. Kills the overcharge (15x on a ceiling-heavy call) with **no** x402 v2 / Permit2 migration | Built and tested behind `X402_ROLLING_SETTLEMENT`, **default off**. Durable payer ledger is in (JSON on disk, same model as task-store). Remaining calls: which surface goes first (`/task-request` is wired; `/v1` stays free per ADR 0006), and whether one-call-per-payer bad debt at $1 max is acceptable. **Do not flip the flag on the live box until you accept the lag:** you pay for the last call; `/task-quote` is a forecast. |
...
| 27 | ~~Deploy the cost-plus engine~~ **Done 2026-08-15** — box on `00120d0`, verified 12/12. **One decision left, no deadline: do you want cost-plus on?** It roughly halves list prices (a median agent call $0.2062 → $0.1034). Prices are unchanged until you set `X402_COST_PLUS=true` alongside `VI_TIER2_MIN_COGS=2000000` and restart; re-run the verifier after, and revert the line if the pricing check fails. With 0 tasks/24h this governs approximately no revenue either way, so item 4 (outreach) still gates everything downstream | Deploy checklist is the item 3c table below. Worth knowing the traffic context before pricing another round: `/stats` on 2026-08-15 read **6 paid tasks in 7d, $0 USDC fees in 7d, 0 tasks in 24h** — the pricing model currently governs approximately no revenue, so item 4 (outreach) gates everything downstream of it |
