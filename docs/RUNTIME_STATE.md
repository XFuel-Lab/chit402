...
- OpenRouter upstream is in the gateway and **off on this host until `OPENROUTER_API_KEY` is set**. No key means no `openrouter/…` rows and no startup error. With the key, quotes are that hub's per-token price + 1% + the $0.002 receipt.
