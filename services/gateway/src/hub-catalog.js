 ...
 * the id that will serve is not the name the caller sent. `xfuel/auto`,
 * typed family names (`deepseek`, `llama`), and exact `openrouter/*` catalog
 * ids are not table hits — those ids are never flagged substituted.
 *
...
export function modelSubstitution(requested, served) {
  ...
  // openrouter/<vendor>/<model> is the catalog id itself.
  if (requestedName.toLowerCase().startsWith('openrouter/')) {
    return {
...
export const SUBSTITUTION_POLICY = Object.freeze({
  ...
  note: 'Names in aliases and alias_patterns are served by the listed target. '
    ...
    + 'Exact openrouter/* catalog ids are never substitutions.',
});
 ...
 *   Exact catalog ids (including `openrouter/*`), `xfuel/auto`, typed family names,
 *   and house-resale familiar names still resolve. `openrouter/*` is never a substitution.
...
export function resolveCatalogModel(modelId, models, opts = {}) {
  ...
  if (
    ...
    && !lower.startsWith('akash/')
    && !lower.startsWith('xfuel/')
  ...
  // Flag off returns null even when rows are listed, so gpt-oss aliases stay off OpenRouter.
  // Strict does not skip this: house resale is its own flag. Exact openrouter/* ids
  // already matched above and are never treated as substitutions.
  if (!hit) {
