...
| `activation` | Quantized **SiLU/GeLU** lookup table + `prove`/`verify` — discharges the FFN's activation obligation soundly. `prove_committed`/`verify_committed` make it succinct and **tie the committed table to the canonical activation table** (M5.4b). |
...
| `ffn` | **SwiGLU FFN sub-block** — (quantized) RMSNorm + 3 matmul proofs + gating Hadamard + (quantized) activation lookup under one transcript, manifest-driven. **Zero pending obligations in quantized mode.** `prove_committed_ffn`/`verify_committed_ffn` (M5.4b) make it **fully succinct**: norm → gate/up (io) → committed requant → committed activation → hadamard-io → down (io) → residual, threaded by commitment reuse — the verifier holds only the block I/O commitments + public weights. |
...
its output. **Committed attention** (`attention::prove_committed_attention`) assembles all of
...
commitments + the public weights; every seam is threaded by commitment reuse (M5.4b). Its sibling
**committed FFN** (`ffn::prove_committed_ffn`) does the same for the SwiGLU path — `norm → gate/up (io) →
