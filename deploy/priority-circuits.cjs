 ...
 *
 * Usage:
 *   npx hardhat run deploy/priority-circuits.cjs --network theta-testnet
 *   npx hardhat run deploy/priority-circuits.cjs --network bittensor-evm
 *   npx hardhat run deploy/priority-circuits.cjs --network hardhat
 */
...
async function main() {
  ...
  // ─── Save Manifest ────────────────────────────────────────────────────────
  const manifestPath = path.join(__dirname, '..', `deploy-priority-${network}-${Date.now()}.json`);
