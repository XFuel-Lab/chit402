 ...
 *
 * Usage:
 *   npx hardhat run deploy/legacy/theta-inference.cjs --network hardhat
 *   npx hardhat run deploy/legacy/theta-inference.cjs --network theta-testnet
 *   npx hardhat run deploy/legacy/theta-inference.cjs --network theta-mainnet
 *
...
async function main() {
  ...
  const manifestPath = path.join(manifestDir, `deploy-theta-inference-${network}-${Date.now()}.json`);
