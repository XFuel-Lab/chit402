...
contract TAOCircuit is AccessControl, Pausable, ReentrancyGuard, IMessageRecipient {
    ...
    // ─── Theta GPU Routing Hook (Hybrid Evolution) ──────────────────────────
    /// @notice ThetaGPUCircuit or ThetaInferenceCircuit for cost-optimized routing.
    /// Per research/theta-integration.md §5.2: When a TAO inference request is
    /// submitted, compare cost between Bittensor subnets and Theta EdgeCloud.
    /// If Theta is cheaper, route to the Theta circuit via internal call.
    address public thetaRouter;
    /// @notice Price per inference on Theta EdgeCloud (admin-set, 18 decimals).
    uint256 public thetaInferencePrice;
    /// @notice Whether Theta routing is enabled.
    bool public thetaRoutingEnabled;
    /// @notice Tasks routed to Theta for cross-circuit analytics.
    uint256 public thetaRoutedCount;
    ...
    /// @notice Emitted when a task is routed to Theta GPU for cost-optimized execution.
    event TaskRoutedToTheta(
        bytes32 indexed taskId,
        address thetaRouter,
        uint256 thetaPrice,
        uint256 taoPrice
    ...
    function submitTask(
        ...
        // If Theta routing is enabled and an inference request, check if Theta
        // offers a cheaper rate. Route to Theta circuit if cost-advantageous.
        if (thetaRoutingEnabled && thetaRouter != address(0)
            && taskType == TaskType.InferenceRequest
            && thetaInferencePrice > 0)
        {
            (uint256 taoPrice, ) = getPrice();
            if (thetaInferencePrice < taoPrice) {
                thetaRoutedCount++;
                emit TaskRoutedToTheta(taskId, thetaRouter, thetaInferencePrice, taoPrice);
            }
        }
    ...
    /// @notice Configure Theta GPU routing for cost-optimized inference.
    /// Per research/theta-integration.md §5.2: compare Bittensor vs Theta pricing.
    function setThetaRouter(address _router) external onlyRole(DEFAULT_ADMIN_ROLE) {
        thetaRouter = _router;
    }

    function setThetaInferencePrice(uint256 _price) external onlyRole(OPERATOR_ROLE) {
        thetaInferencePrice = _price;
    }

    function setThetaRoutingEnabled(bool _enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
        thetaRoutingEnabled = _enabled;
    }
